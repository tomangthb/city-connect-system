    ...
    <Sidebar className="border-r border-sidebar-border bg-sidebar theme-transition">
      <SidebarHeader className="p-4 border-b border-sidebar-border bg-sidebar">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center theme-transition">
                ...
                <SidebarMenuItem key={item.url}>
                  <SidebarMenuButton 
                    ...
                    className={`
                      ...
                      ${activeTab === item.url 
                        ? 'bg-green-100 dark:bg-green-900/30 text-green-900 dark:text-green-100 border-l-4 border-green-600 shadow-sm' 
                        : 'text-sidebar-foreground hover:bg-green-50 dark:hover:bg-green-900/20 hover:text-green-700 dark:hover:text-green-300'
                      }
                    `}
                  >
                    <item.icon className={`h-5 w-5 ${activeTab === item.url ? 'text-green-600 dark:text-green-400' : 'text-sidebar-foreground/70'}`} />
                    <span className={`${activeTab === item.url ? 'font-medium' : ''}`}>
                      {item.title}
                    </span>
                  </SidebarMenuButton>
                </SidebarMenuItem>
